...
export function checkSystemDeps(deps, platform) {
  ...
  const allRequiredFound = !hasAnyRequired || checks.every((c) => c.level !== 'required' || c.found);
  const missingRecommendedOnly = allRequiredFound && checks.some((c) => !c.found && c.level === 'recommended');
  const hasFoundRequired = checks.some((c) => c.level === 'required' && c.found);
  const status = missing.length > 0
    ? 'block'
    : (missingRecommendedOnly && !hasFoundRequired)
      ? 'warn'
      : 'ok';
