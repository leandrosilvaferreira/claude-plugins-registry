...
- [cross-platform-artifacts](cross-platform-artifacts.md) — artefatos gerados devem ser .mjs, nunca .sh/.cmd/.bat/.ps1/.py — plugin é cross-platform (Win/Mac/Linux)
...
- [node-runtime-nvm](node-runtime-nvm.md) — nesta máquina de dev, node não está no PATH em shells não-interativos; usar path do nvm ou resolver
- [node-resolution-target-machine](node-resolution-target-machine.md) — artifacts .mjs no projeto-alvo não podem assumir 1 gerenciador de node: nativo, nvm ou fnm
