...
| **`check-consistency`** `[path]` | Whole-harness consistency audit: a deterministic pass resolves every cross-reference between skills, agents, rules, commands, hooks and every `CLAUDE.md` (root and nested), flagging references that point at nothing and hook scripts never wired in `settings.json`; parallel `sonnet` subagents then judge the ambiguous references and whether each artifact's content still fits the project's real stack. Consolidates one approval plan, applies approved fixes via parallel subagents, re-verifies deterministically. |
