...
**The `## Rules` block is fixed — reproduce it verbatim**, including the `aia-harness:fixed`
sentinel comment and both bullets exactly as shown above, on every domain, every run. It must
stay byte-identical to `DOMAIN_FIXED_RULES` in `lib/generate/claude-md.mjs`, never derived,
reworded, shortened, or omitted — `doctor`'s "Fixed rules intact" check greps for this marker
and flags any domain CLAUDE.md missing it as a regression. Being identical across every domain
is correct **by design**; do not treat it as something to make "domain-specific" like the
sections above it.
...
- No section is a copy-paste of root CLAUDE.md content without domain-specific adaptation
  (the fixed `## Rules` section below is exempt — identical across domains by design)
- `## Rules` is present, verbatim: heading, `aia-harness:fixed` sentinel comment, and both
  baseline bullets unchanged — never reworded, shortened, or omitted
