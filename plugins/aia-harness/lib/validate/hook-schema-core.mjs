/**
 * Validators for the hook event types this harness currently ships hooks
 * for (PreToolUse, PostToolUse family, Stop family, UserPromptSubmit,
 * PermissionRequest, SessionStart, SubagentStart, Setup, Notification,
 * SessionEnd, PreCompact). Re-exported from hook-schema.mjs, the public
 * entry point — import from there, not this file directly.
 *
 * @module validate/hook-schema-core
 */
 ...
 *
 * @param {string} stdout @param {number} exitCode
 * @param {string} [eventName] discriminant to check hookSpecificOutput.hookEventName against — "Stop" or "SubagentStop"
 * @returns {ValidationResult}
 */
export function validateStopOutput(stdout, exitCode, eventName = "Stop") {
  const r = parseStdout(stdout, exitCode);
  if (!r.ok) return r.result;
  const { obj } = r;
  /** @type {string[]} */
  ...
  if ("hookSpecificOutput" in obj) {
    errors.push(
      ...checkHookSpecificOutput(obj.hookSpecificOutput, eventName, ["additionalContext"]),
    );
  }
  errors.push(...validateCommonFields(obj));
  return { valid: errors.length === 0, errors };
...
/**
 * Validates SubagentStop hook output. Identical schema to Stop, except its
 * hookSpecificOutput.hookEventName discriminant (when present) is "SubagentStop".
 * @param {string} stdout @param {number} exitCode @returns {ValidationResult}
 */
export function validateSubagentStopOutput(stdout, exitCode) {
  return validateStopOutput(stdout, exitCode, "SubagentStop");
}
...
export function validateUserPromptSubmitOutput(stdout, exitCode) {
  ...
  if ("hookSpecificOutput" in obj) {
    const hso = obj.hookSpecificOutput;
    errors.push(
      ...checkHookSpecificOutput(hso, "UserPromptSubmit", ["additionalContext", "sessionTitle"]),
    );
    if (typeof hso === "object" && !Array.isArray(hso) && hso !== null) {
      errors.push(...checkBooleanField(hso, "suppressOriginalPrompt"));
    }
  }
  errors.push(...validateCommonFields(obj));
  return { valid: errors.length === 0, errors };
...
const PERMISSION_REQUEST_BEHAVIORS = new Set(["allow", "deny"]);
 ...
 *   decision: { behavior: "allow", updatedInput?: object, updatedPermissions?: array }
  ...
  if ("hookSpecificOutput" in obj) {
    ...
    if (baseErrs.length === 0 && typeof hso === "object" && !Array.isArray(hso) && hso !== null) {
      if (!("decision" in hso)) {
        ...
        if (!("behavior" in decision) || !PERMISSION_REQUEST_BEHAVIORS.has(decision.behavior)) {
          errors.push(
            `hookSpecificOutput.decision.behavior must be "allow" or "deny", got "${decision.behavior}"`,
          );
        }
        errors.push(...checkObjectField(decision, "updatedInput"));
        if ("updatedPermissions" in decision && !Array.isArray(decision.updatedPermissions)) {
          errors.push("hookSpecificOutput.decision.updatedPermissions must be an array");
        }
        if ("message" in decision && typeof decision.message !== "string") {
          errors.push(
            `hookSpecificOutput.decision.message must be a string, got ${typeof decision.message}`,
          );
        }
        errors.push(...checkBooleanField(decision, "interrupt"));
      }
    }
  }
 ...
 *   initialUserMessage?: string, sessionTitle?: string, watchPaths?: string[],
...
export function validateSessionStartOutput(stdout, exitCode) {
  ...
  if ("hookSpecificOutput" in obj) {
    const hso = obj.hookSpecificOutput;
    errors.push(
      ...checkHookSpecificOutput(hso, "SessionStart", [
        "additionalContext",
        "initialUserMessage",
        "sessionTitle",
      ]),
    ...
    if (typeof hso === "object" && !Array.isArray(hso) && hso !== null) {
      ...
      errors.push(...checkBooleanField(hso, "reloadSkills"));
    }
  }
  errors.push(...validateCommonFields(obj));
  return { valid: errors.length === 0, errors };
